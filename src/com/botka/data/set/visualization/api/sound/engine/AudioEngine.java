...
/**
 * Base class that represents the audio engine of the system.
 *
 * @author Jake Botka
 *
 */
public class AudioEngine {
	...
	/**
	 * Plays audio by retrieving the file from the hashap of audio files using the
	 * key ID
	 * 
	 * @param ID assigned to the AudioFile object connecting to the file inside the
	 *           hashmap
	 */
	...
	/**
	 * Plays audio dirrectly from the file Does not need to retrieve from hashmap.
	 * If the file is inside the hashmap it will retrieve it ID and send it to the
	 * callback interface method onAudioPlayed(id). if not the ID is passed as -1
	 * (this is done in the method getIDfromfile()).
	 * 
	 * @param audio file
	 */
